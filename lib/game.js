...
var Game = {
    ...
    update: function () {
        ...
        models.buildings_full.find({}).each(function (building) {
            switch (building.status)
            {
                ...
                case 1:
                    building_props[building.type]['resources_out'].forEach(function (resource) {
                        var count = 0;
                        type = models.resources.types[resource.type];
                        if (building.field.resources[type + '_c'] > resource.count) {
                            count = resource.count;
                            building.field.resources[type + '_c'] -= resource.count;
                        } else {
                            count = building.field.resources[type + '_c'];
                            building.field.resources[type + '_c'] = 0;
                        }
                        building.addGoods(resource.type, count, building.field.resources[type + '_q']);
                    });
                    break;
            }
            building.save();
